...
//				canvas.setPixelSize(width, height);
	...
	public static Integer calItemsSize(int initWidth, int maxWidth, int marginSize) {
		int newWidth = initWidth;
		int colNum = 1;
		
		while (newWidth * colNum < (maxWidth - marginSize*colNum*2)) {
			colNum++ ;
		}
		int minus = maxWidth - colNum * newWidth * marginSize*2;
		if(minus < 0) {
			colNum--;
		} 
		newWidth= (maxWidth-colNum*2*marginSize)/colNum;
		return newWidth;
	}
}
